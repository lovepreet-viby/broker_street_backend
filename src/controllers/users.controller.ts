...
export const userSignUp = async (
    ...
    try {
        ...
        let checkObject = Object.keys(userData).filter(
            (o) => !(userData as any)[o]
        );
        ...
        return res
            .status(200)
            .send({ hash: otpData.hash, phoneNumber: data.phoneNumber });
    } catch (err) {
        console.log(err);
        res.status(500).send("Something went wrong!");
    }
...
export const checkUserOtp = async (
    ...
    try {
        ...
        if (data.newuser == true) {
            let checkPhoneNumber = (await checkByPhoneNumber(
                userData.phoneNumber
            )) as any;
            if (checkPhoneNumber) {
...
export const userLogin = async (
    ...
    try {
        ...
        let isCheckPhoneNumber = (await checkByPhoneNumber(phoneNumber)) as {
            _id: string;
            phoneNumber: number;
            role: string;
            lastName: string;
        ...
        if (!isCheckPhoneNumber) {
            return res
                .status(400)
                .send("There was no account on this phone number");
        }
        // To do
        // otp send service
