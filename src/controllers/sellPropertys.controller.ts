...
import {
    createSellPropertyDetail,
    getAllSellPropertyList,
    getSellPropertyDetail,
    updateSellPropertyDetail,
...
export const getAllSellProperty = async (
    ...
    try {
        ...
        let sellPoperty = (await getAllSellPropertyList(
            page,
            limit,
            searchkey,
            userId
...
export const deleteSellProperty = async (
    ...
    try {
        ...
        let checkSellProperty = (await checkSellPropertyId(
            sellPropertyId
        )) as any;
        ...
        if (checkSellProperty.userId.toString() != req.body.user.user_id) {
            return res
                .status(400)
                .send(
                    "Unauthorized person, so you cannot delete the property."
                );
        }

        let buyPoperty = (await deleteSellPropertyDetail(
            sellPropertyId
        )) as any;
        if (!buyPoperty) {
            return res.status(400).send(false);
...
export const getAllUserSellProperty = async (
    ...
    try {
        ...
        let sellPoperty = (await getAllUserSellPropertyList(
            page,
            limit,
            userId
        )) as any;
